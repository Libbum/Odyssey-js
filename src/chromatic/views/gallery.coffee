...
class Chromatic.GalleryView
  ...
  layout: =>
    ...
    if rows < 1
      ...
      # console.log(weights)
      # console.log(rows)
      # worker = cw(_linear_partition)
      # worker.data([weights,rows]).then((a) ->
      #    console.log(a)
      #    worker.close()
      #    return)
      console.log(weights.join() + rows)
      p = new Parallel({weights: weights, rows: rows},  {evalPath: 'assets/js/eval.js'})
      p.require('underscore-parts.js')
      p.spawn((data) ->
         _cache = {}
         seq = data.weights
         k = data.rows
         test = (seq, k) ->
           ...
           _cache[key] = [seq[i] for i in [0...n+1]].concat ans
           return
       # partition = _linear_partition(data.weights, data.rows)
        console.log(test)
        return)
        .then((data) ->
      ...
      # worker = new Worker("assets/js/script1.js")
      #
      ...
      #    #   index += row.length)
      # console.log(@photos)
      # #console.log(@photo_views)
      # worker.postMessage({weights: weights, rows: rows, photos: @photos, photo_views: @photo_views})
