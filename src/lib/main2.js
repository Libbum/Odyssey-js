   ...
   (function() {
      function grab(t, n, e) {
         ...
         t.append("path").datum({
            ...
            var n;
            if (e) n = t.insert("path", ".iglobe-foreground").datum({
               type: "Point",
      ...
      function gotoView(coords) {
         ...
         d3.transition().delay(1500).duration(2000)
            .tween("rotate", function() {
               ...
               var sc = d3.interpolate(proj.scale(), 190);
               return function(i) {
      ...
      var proj = d3.geo.orthographic().precision(0.5).clipAngle(90).clipExtent([
            [-1, -1],
            [401, 401]
         ]).translate([200, 200]).scale(190).rotate([-40, -30]),
         r = d3.dispatch("world"),
         c = -1,
      ...
      d3.selectAll("#map").data([proj]).append("svg").attr("preserveAspectRatio", "xMinYMin meet").attr("viewBox", "0 0 400 400").each(function(p) {
         ...
         r.on("world." + c++, function() { a.selectAll("path").attr("d", e); });
      });
